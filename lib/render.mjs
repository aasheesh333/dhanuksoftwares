...
// Keeps anchor markup balanced: drops orphan </a> tags, drops nested <a>
// openers, and auto-closes an unclosed <a>. Without this, a single unclosed <a>
// makes the browser swallow every following heading and paragraph into the
// link, so the whole article renders as one underlined, clickable block.
function balanceAnchors(html) {
  if (!html) return html;
  const tokens = String(html).split(/(<\/?a\b[^>]*>)/i);
  let out = '';
  let open = false;
...
function renderRelatedPostsHtml(posts) {
  if (!posts || !posts.length) return '';
  return posts.map(p => {
    const title = escapeHtml(p.title || '');
    const cat = escapeHtml(p.category || 'Guide');
    let desc = (p.description || p.tagline || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
    if (desc.length > 155) desc = desc.slice(0, 152).trim() + '…';
    return `      <a class="related-post-card" href="/blog/${escapeHtml(p.slug)}/"><div class="related-post-tag">${cat}</div><div class="related-post-title">${title}</div><div class="related-post-desc">${escapeHtml(desc)}</div><span class="related-post-go">Read guide &rarr;</span></a>`;
  }).join('\n');
}

export function renderApp(app, baseUrl, relatedApps, relatedPosts = []) {
  ...
  html = html.replace(/\{\{#if relatedPosts\.length\}\}([\s\S]*?)\{\{\/if\}\}/g, (m, body) =>
    (relatedPosts && relatedPosts.length) ? body : ''
  );
  ...
  html = html.replace(/\{\{#each relatedPosts\}\}[\s\S]*?\{\{\/each\}\}/g, () => renderRelatedPostsHtml(relatedPosts));
