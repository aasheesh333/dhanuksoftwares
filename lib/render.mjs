...
export function buildSchemaSoftwareApp(app, baseUrl) {
  ...
  if (primaryImage) {
    ...
    schema.image = `${baseUrl}/og-banner.png`;
    schema.screenshot = [`${baseUrl}/og-banner.png`];
  }
...
function renderScreenshotsHtml(screenshots, appName) {
  ...
  return screenshots.map((s, i) => `      <img src="${escapeHtml(s)}" alt="${escapeHtml(appName)} screenshot ${i + 1}" loading="lazy" width="540" height="1170"/>`).join('\n');
}
...
function renderRelatedHtml(related) {
  ...
  return related.map(r => {
    const emoji = r.emoji ? `<div class="related-emoji">${escapeHtml(r.emoji)}</div>` : '';
    return `      <a class="related-card" href="/apps/${escapeHtml(r.slug)}/">${emoji}<div class="related-name">${escapeHtml(r.name)}</div><div class="related-desc">${escapeHtml(r.shortDesc || '')}</div></a>`;
  }).join('\n');
}
...
export function renderApp(app, baseUrl, relatedApps) {
  ...
  const hasStats = !!(app.totalDownloads || (app.rating && app.rating.count > 0) || app.appSize || app.contentRating || app.lastUpdated);
  ...
  html = html.replace(/\{\{#if rating\.value\}\}([\s\S]*?)\{\{\/if\}\}/g, (m, body) =>
    (app.rating && app.rating.value) ? body : ''
  );
  ...
  if (secondary.length > 0) {
    ...
    // Remove the entire {{#if hasSecondaryDownloads}}...{{/if}} block including wrapper
    html = html.replace(/\{\{#if hasSecondaryDownloads\}\}[\s\S]*?\{\{\/if\}\}/g, '');
    // Remove any bare {{#each secondaryDownloads}}...{{/each}} blocks (no items)
    html = html.replace(/\{\{#each secondaryDownloads\}\}[\s\S]*?\{\{\/each\}\}/g, '');
    // Also strip any orphan closing wrapper tags left by prior regex edge cases
    html = html.replace(/<div class="more-stores">[\s\S]*?<\/details>\s*<\/div>/g, '');
  }
