...
import { sanitizeRichText } from './render.mjs';
...
export function renderBlogPost(post, baseUrl, apps = []) {
  ...
  <section class="blog-faq" style="margin-top:3rem;">
    <h3 style="font-family:'Inter',sans-serif;font-size:1.5rem;font-weight:700;color:#fff;margin-bottom:1.5rem;">Frequently Asked Questions</h3>
    ${faqItems}
  </section>`;
  }
  ...
  let html = template
    ...
    .replace(/\{\{contentHtml\}\}/g, sanitizeRichText(post.contentHtml || ''))
...
export function renderBlogIndex(posts, baseUrl) {
  const cards = posts.map(post => {
    ...
    <article class="blog-card" style="background:var(--card);border:1px solid var(--border);border-radius:16px;padding:2rem;display:flex;flex-direction:column;transition:border-color 0.2s,transform 0.2s;">
      ...
      <h2 style="font-family:'Inter',sans-serif;font-size:1.4rem;font-weight:700;margin-bottom:0.75rem;line-height:1.35;">
...
<head>
  ...
  <style>
    ...
    .blog-header h1 { font-family: 'Inter', sans-serif; font-size: clamp(2.2rem, 5vw, 3.2rem); font-weight: 800; color: #fff; margin-bottom: 1rem; }
